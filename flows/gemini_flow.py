...
def run_gemini_task(page, prompt, task_type, out_dir, platforms=None):
    # 1. Navigation
    page.goto("https://gemini.google.com/", wait_until="domcontentloaded", timeout=60000)
    
    box_selector = "div[role='textbox']"
    page.wait_for_selector(box_selector, timeout=30000)
    ...
    page.fill(box_selector, f"{prefix}{prompt}")
    ...
    if task_type == "PPT":
        ...
        try:
            with page.context.expect_page(timeout=120000) as new_page_info:
            ...
            page.wait_for_selector(text_selector, timeout=10000)
            content = page.locator(text_selector).last.inner_text()
    ...
    # 4. Video Handling Logic (Robust Update)
        ...
        try:
            time.sleep(5) 
            if page.locator(generate_btn_selector).is_visible():
                print("Clicking 'Generate' button inside Gemini...")
                page.locator(generate_btn_selector).click()
            ...
            page.wait_for_selector(video_selector, timeout=420000, state="attached") 
            ...
            page.wait_for_selector(download_btn_selector, timeout=60000)
            
            print("Attempting to download video...")
            try:
                # force=True ignores the 'pointer-events' check that failed previously
                with page.expect_download(timeout=120000) as download_info:
                    page.locator(download_btn_selector).last.click(force=True)
                ...
                if 'youtube' in [p.lower() for p in (platforms or [])]:
                    try:
                        ...
                        text_selector = "div[class*='message-content'], .model-response-text"
                        page.wait_for_selector(text_selector, timeout=5000)
                        response_text = page.locator(text_selector).last.inner_text()
    ...
    # 5. Image Handling Logic (Fully Automated)
        ...
        try:
            page.wait_for_selector(image_selector, timeout=150000)
            print("Image content detected.")
            
            download_btn = "button[aria-label*='Download'], [mattooltip*='Download']"
            time.sleep(5) 
            
            page.locator(image_selector).first.hover()
            
            with page.expect_download(timeout=60000) as download_info:
                page.locator(download_btn).first.click(force=True)
            ...
            page.screenshot(path=os.path.join(out_dir, "image_error.png"))
